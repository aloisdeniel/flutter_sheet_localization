...
class SheetLocalizationGenerator extends GeneratorForAnnotation<SheetLocalization> {
  ...
  FutureOr<String> generateForAnnotatedElement(Element element, ConstantReader annotation, BuildStep buildStep) async {
    ...
    final localizationDelegateSupertype =
        classElement.allSupertypes.firstWhere((x) => x.name == 'LocalizationsDelegate');
    if (localizationDelegateSupertype != null) {
      ...
      localizations = localizations.copyWith(name: classElement.name.replaceAll('Delegate', ''));

      final builder = DartBuilder();
      return builder.build(localizations);
    }
    throw InvalidGenerationSourceError('Supertype aren\'t valid : [${classElement.allSupertypes.join(', ')}].',
        todo: 'Define only one supertype of type LocalizationsDelegate<LOCALIZATION_CLASS_NAME>.', element: element);
  }

  Future<Localizations> _downloadGoogleSheet(String? documentId, String? sheetId) async {
